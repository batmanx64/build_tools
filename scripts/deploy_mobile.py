...
# -*- coding: utf-8 -*-
"""
deploy_mobile.py - Mobile 部署脚本
===================================
功能：将移动平台构建产物部署到 out/<platform>/<branding>/mobile 目录。
支持 iOS 和 Android 平台，包括 x2t 引擎、字体、JS 资源、字典等。
Android 版还会聚合多 ABIs（arm64-v8a、armeabi-v7a、x86、x86_64）。
"""
...
def exclude_arch(directory, frameworks):
    """从 fat binary 中移除 arm64 架构（用于模拟器版本）。"""
...
def deploy_fonts(git_dir, root_dir, platform=""):
    ...
    # Android 额外需要 DejaVu 和 Liberation 字体
    if (platform == "android"):
...
def make():
    ...
    # ============ Android 多 ABI 聚合 ============
    # Android 需要将多个架构的 .so 文件聚合到同一个输出目录中
    for native_platform in platforms:
