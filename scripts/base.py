...
# -*- coding: utf-8 -*-
"""
base.py - ONLYOFFICE 构建系统基础工具库
========================================
功能：提供构建过程中最常用的工具函数，包括：
  - 文件系统操作（读写、复制、删除、移动）
  - 系统命令执行
  - Git 仓库管理（克隆、更新、拉取）
  - Qt/QMake 环境配置
  - 平台检测
  - ICU 库处理
  - macOS/Linux RPATH 修正
  - SDKJS 插件管理
  - 版本号管理

该模块是所有其他构建脚本的基础依赖，约 2100 行。
"""
...
# ICU 版本号常量
icu_ver = "74"
icu_ver_old = "58"  # 用于 Windows XP 兼容
...
def configure_common_apps(file=""):
    """将构建工具（7z, curl, vswhere）的路径添加到 PATH 环境变量。"""
    if ("windows" == host_platform()):
...
def check_build_version(dir):
    """从 version 文件中读取产品版本号并设置 PRODUCT_VERSION 环境变量。"""
...
def copy_dir_windows(src, dst):
    """Windows 平台专用：使用 robocopy 复制目录（支持长路径）。"""
    if is_dir(dst):
        delete_dir(dst)
...
def delete_dir_with_access_error(path):
    """
    递归删除目录，处理权限错误（如 Windows 上设置了只读的文件）。
    对长路径（>260 字符）使用 \\?\ 前缀（Windows 专用）。
    """
...
def copy_lib(src_folder, dst, name, subdir=""):
    """
    复制库文件（支持 .so, .dll, .dylib, .a, .lib）。
    macOS 上支持 .framework 和 .xcframework 的复制与生成。
    """
    src = src_folder
    if subdir != "":
        src += ("/" + subdir)
    ...
    if (config.check_option("config", "bundle_dylibs")) and is_dir(src + "/" + name + ".framework"):
        ...
        # 如果还需要 xcframework，创建模拟器 + 真机通用框架
...
def cmd(prog, args=[], is_no_errors=False):
    """
    执行系统命令。
    is_no_errors=True 表示忽略执行错误（不会终止进程）。
    """
    ret = 0
    if ("windows" == host_platform()):
        sub_args = args[:]
...
def cmd_exe(prog, args, is_no_errors=False):
    """
    执行可执行文件（自动设置 LD_LIBRARY_PATH / DYLD_LIBRARY_PATH）。
    用于执行那些依赖特定动态库的程序。
    """
...
def cmd_in_dir_qemu(platform, directory, prog, args=[], is_no_errors=False):
    """通过 QEMU 模拟器执行命令（用于 arm64 交叉编译环境）。"""
    platform_config = {
        "linux_arm64": {
            "qemu": "qemu-aarch64",
...
def create_qemu_wrapper(binary_path, platform):
    """
    为指定平台创建 QEMU 包装器脚本。
    将原始二进制文件重命名为 .bin，然后创建 shell 包装脚本通过 QEMU 调用它。
    用于在非 arm64 宿主机上执行 arm64 二进制文件。
    """
...
def git_get_origin():
    """获取当前 build_tools 仓库的 git remote origin URL。"""
    cur_dir = os.getcwd()
...
def git_get_base_url():
    """获取 Git 基础 URL。优先从 origin URL 提取，否则默认使用 GitHub。"""
...
def get_ssh_base_url():
    """从当前 origin URL 获取 SSH 格式的基础 URL。"""
...
def git_update(repo, is_no_errors=False, is_current_dir=False, git_owner=""):
    ...
    # update-light 模式下不切换分支，避免丢失本地修改
...
def get_repositories():
    """
    获取需要更新的所有仓库列表。
    根据 --module 参数动态包含 server/desktop/builder 所需的仓库。
    """
    result = {}
...
def create_pull_request(branches_to, repo, is_no_errors=False, is_current_dir=False):
    """
    创建跨分支的 Pull Request。
    将当前分支的代码合并到指定的一个或多个目标分支。
    使用 gh CLI 创建 PR，如果有冲突则自动中止。
    """
...
def get_compiler_gcc_prefix(platform):
    """
    获取 GCC 交叉编译器前缀。
    例如 arm64 交叉编译返回 "aarch64-linux-gnu-"，普通架构返回 "/usr/bin/"。
    """
    directory = "/usr/bin"
...
def get_gcc_version():
    """获取 GCC 主版本号（主版本×1000 + 次版本）。使用 sysroot 时返回固定值 5.4。"""
    if config.option("sysroot") != "":
        return 5004
    gcc_path = "gcc"
    gcc_version_major = 4
...
def qt_setup(platform):
    ...
    # ARM Linux 回退到 x86_64 Qt（如果没有 ARM 版本）
...
def check_congig_option_with_platfom(platform, option_name):
    """
    检查配置选项是否适用于指定平台。
    支持跨平台配置选项（config_addon_windows/config_addon_linux 等）。
    """
...
def correct_makefile_after_qmake(platform, file):
    """
    修正 Android qmake 生成的 Makefile 中的库文件名。
    Android NDK 使用不同的架构标识符（arm64-v8a → .a, .so）。
    """
...
def qt_config(platform):
    """
    构建传递给 qmake 的 CONFIG 参数。
    包括模块选择、调试/发布模式、平台特性、预编译头控制等。
    """
...
def qt_copy_lib(lib, dir):
    """复制 Qt 库到目标目录。处理版本号后缀和 Framework。"""
    qt_dir = get_env("QT_DEPLOY")
...
def qt_copy_plugin(name, out):
    """复制 Qt 插件到输出目录。处理 debug/release 版本的 DDL。"""
...
def app_make():
    """获取当前平台的 make 工具名：nmake（Windows）/ make（Unix）。"""
    if is_windows():
        return "nmake"
    return "make"
...
def generate_doctrenderer_config(path, root, product, vendor = "", dictionaries = ""):
    """为 doctrenderer 生成 XML 配置文件，指定 native.js、字体、JS 等资源路径。"""
    content = "<Settings>\n"
...
def generate_plist(file, platform):
    """生成 macOS/iOS Framework 的 Info.plist 文件。"""
...
def generate_xcprivacy(file, platform):
    """生成 Apple PrivacyInfo.xcprivacy 文件。"""
    content = \
...
def for_each_framework(path, platform, callbacks, max_depth=512):
    """递归遍历目录中的所有 .framework 文件并执行回调函数。"""
...
def correct_bundle_identifier(bundle_identifier):
    """清理 bundle identifier（替换非法字符为连字符）。"""
...
def extract_unicode(src, dst, is_no_errors=False):
    """使用支持 Unicode 路径的方式解压文件（Windows 专用）。"""
    if "windows" == host_platform():
...
# ============================================================
# Windows vcvarsall（Visual Studio 环境配置）
# ============================================================

def _call_vcvarsall_and_return_env(arch):
    """
    调用 vcvarsall.bat 并捕获设置后的环境变量（INCLUDE, LIB, LIBPATH, PATH）。
    用于在构建时设置 Visual C++ 编译环境。
    """
...
def run_as_bat_win_isolate(lines, is_no_errors=False):
    """在独立窗口中执行批处理（使用 start /wait /min 避免阻塞 UI）。"""
...
def mac_correct_rpath_binary(path, libs):
    """将库引用从绝对路径改为 @rpath 相对路径。Framework 模式下跳过。"""
    try:
        if config.check_option("config", "bundle_dylibs"):
            return
    except:
...
def mac_add_loader_path_to_rpath(libs):
    """为库添加 @loader_path 到 rpath。Framework 模式下调整路径为 ../../../。"""
    for lib in libs:
        if config.check_option("config", "bundle_dylibs"):
...
def mac_correct_rpath_desktop(dir):
    """修正 Desktop Editors 的 @rpath 链接。包括编辑器主程序、helper 进程等。"""
    ...
    # 创建 GPU 和 Renderer 专用 helper app 副本
...
def linux_set_origin_rpath_libraries(dir, libs):
    """使用 patchelf 设置 Linux 共享库的 $ORIGIN rpath。"""
...
def copy_sdkjs_plugin(src_dir, dst_dir, name, is_name_as_guid=False, is_desktop_local=False):
    ...
    if not is_name_as_guid:
        ...
        return
    # 使用 GUID 作为目录名（从 config.json 读取 asc.{GUID}）
    if not is_file(src_dir_path + "/config.json"):
        return
...
def hack_xcode_ios():
    """Hack Xcode iOS qmake 规格文件以修复多架构编译问题。"""
...
def make_sln(directory, args, is_no_errors):
    """在 Windows 上使用 devenv.com 编译 Visual Studio 解决方案。"""
...
def make_sln_project(directory, sln_path):
    """编译 Visual Studio 解决方案项目的所有配置（x64 和 Win32）。"""
    args = []
...
def copy_v8_files(core_dir, deploy_dir, platform, is_xp=False):
    """复制 V8 JavaScript 引擎的 ICU 数据文件到部署目录。"""
...
def restorePathForBuilder(new_path):
    """恢复被 correctPathForBuilder 修改的文件。"""
    if ("" == new_path):
        return
    old_path = new_path[:-4]
...
def convert_ios_framework_to_xcframework(folder, lib):
    """将 iOS Framework 转换为 XCFramework（合并真机和模拟器）。"""
    cur_dir = os.getcwd()
    os.chdir(folder)
...
def convert_ios_framework_to_xcframework_folder(folder, libs):
    """批量转换 iOS Frameworks 为 XCFrameworks。"""
    for lib in libs:
...
def change_elf_rpath(path, origin):
    """修改 ELF 二进制文件的 RPATH 以包含 $ORIGIN。"""
...
def is_need_build_js():
    """判断当前是否需要进行 JS 构建（osign 模块不需要 JS）。"""
...
def set_sysroot_env(platform):
    """
    设置 sysroot 编译环境。
    修改 PATH、CC、CXX、CFLAGS 等环境变量以使用 sysroot 中的交叉编译工具链。
    """
...
def check_python():
    """
    检查并确保 Python3 可用。
    如果 tools/linux/python3 不存在，自动下载并解压。
    """
    if ("linux" != host_platform()):
        return
...
def create_artifacts_qemu_win_arm():
    """启动 QEMU Windows ARM64 虚拟机以完成构建。"""
...
def setup_local_qmake(dir_qmake):
    """配置本机 Qt 的 qmake（通过写入 onlyoffice_qt.conf 指定 Prefix）。"""
